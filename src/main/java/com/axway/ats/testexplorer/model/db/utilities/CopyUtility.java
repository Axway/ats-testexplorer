...
public abstract class CopyUtility {
    ...
    public CopyUtility( String srcDbHost, int srcDbPort, String srcDbName, String dstDbHost, int dstDbPort,
                        String dstDbName, String dbUser,
    ...
    private void initConnections() throws DatabaseAccessException {
        ...
        if (srcConnection instanceof DbConnSQLServer) {
            ...
            String errMsg = "Created read connection to source database is of unsuppoted class '"
                            + srcConnection.getClass().getName() + "'";
        ...
        if (dstConnection instanceof DbConnSQLServer) {
            ...
            String errMsg = "Created read connection to destination database is of unsuppoted class '"
                            + dstConnection.getClass().getName() + "'";
            throw new DatabaseAccessException(errMsg);
        }
        ...
        if (dstConnection instanceof DbConnSQLServer) {
            ...
            String errMsg = "Created write connection to source database is of unsuppoted class '"
                            + srcConnection.getClass().getName() + "'";
            throw new DatabaseAccessException(errMsg);
        }
    ...
    private DbConnection createDbConnection( String host, int port, String name, String user,
        ...
        if (DbConnSQLServer.DEFAULT_PORT == port) {
            ...
            throw new DatabaseAccessException("Unable to connect to either MSSQL or PostgreSQL database at '" + host
                                              + ":" + port + "'");
        }
    ...
    protected int copyRun( Run srcRun ) throws DatabaseAccessException, ParseException, NumberFormatException,
    ...
    protected void copyRunMessages( Run srcRun, Run dstRun ) throws DatabaseAccessException, NumberFormatException,
                                                             DbEntityCopyException {
    ...
    protected int copySuite( Suite srcSuite, int dstRunId ) throws DatabaseAccessException, ParseException,
                                                            NumberFormatException, DbEntityCopyException {
    ...
    protected void copySuiteMessages( Suite srcSuite, Suite dstSuite ) throws DatabaseAccessException,
                                                                       NumberFormatException, DbEntityCopyException {
        ...
        if (srcSuiteMessagesCount > 0) {
            ...
            for (int recordsCount = MESSAGES_CHUNK_TO_COPY; stillcopying; recordsCount += MESSAGES_CHUNK_TO_COPY) {
                ...
                srcSuiteMessages = this.srcDbRead.getSuiteMessages(startRecord, recordsCount,
                                                                   "where suiteId=" + srcSuite.suiteId,
                                                                   "suiteMessageId",
                                                                   true, 0);
                startRecord = recordsCount + 1;
    ...
    protected void copyTestcase( Testcase srcTestcase, int dstSuiteId ) throws DatabaseAccessException,
        ...
        Scenario srcScenario = this.srcDbRead.getScenarios(0, 1, "WHERE scenarioId = " + srcTestcase.scenarioId,
                                                           "scenarioId", true, 0)
                                             .get(0);
    ...
    private void copyActionsInfo( String srcTestcaseId, LoadQueue srcActionQueue,
        ...
        for (CheckpointSummary summaryAction : srcSummaryActions) {
            ...
            this.dstDbWrite.populateCheckpointSummary(dstActionQueueId, summaryAction.name,
                                                      summaryAction.transferRateUnit, true);
    ...
    private void copyStatistics( String srcTestcaseId, int dstTestcaseId ) throws DatabaseAccessException,
        ...
        if (srcStatisticDescriptions.size() > 0) {
            ...
            if (srcMachineIdsString.length() > 0) {
                srcMachineIdsString = new StringBuilder(srcMachineIdsString.substring(0, srcMachineIdsString.length()
                                                                                         - 1));
            }
            ...
            for (int srcStatisticTypeId : srcStatisticTypeIds.keySet()) {
                ...
                for (Statistic srcStatistic : srcStatistics) {
                    ...
                    this.dstDbWrite.insertSystemStatistics(dstTestcaseId,
                                                           ...
                                                           AbstractDbAccess.DATE_FORMAT.parse(srcStatistic.getDate())
                                                                                       .getTime(),
                                                           true);
                }
            }
    ...
    private Map<Integer, Integer>
            getMappingOfSrcToDestinationStatTypeIds( List<StatisticDescription> srcStatisticDescriptions ) throws DatabaseAccessException {
